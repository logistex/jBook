...
def myScatter(연도, 알파, fx, fy):
    ...
    plt.scatter(                                             # 산점도 출력 함수
        ...
        c=cctv_with_pop[f'오차 20{연도}'],                   # <f-sting>마커 색상 (오차 데이터를 마커 색상으로 지정)
        s=np.sqrt(cctv_with_pop['CCTV 비율 20'+연도])*1000,  # <문자열 연결> 마커 크기 (해당 연도 CCTV 비율의 제곱근에 비례)
        alpha=알파                                           # 투명도
    )
    # 2) 추세선 그리기
